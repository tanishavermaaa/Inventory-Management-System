...
export default function SupplierSidebar({ onWidthChange }) {
  ...
  // Listen for live order notifications
  useEffect(() => {
    if (!me.id) return;
  ...
  const links = [
    { to: '/supplier-dashboard',         icon: <FaBoxes />,        label: 'Products' },
    { to: '/supplier-dashboard/orders',  icon: <FaShoppingCart />, label: 'Orders'   },
    { to: '/supplier-dashboard/profile', icon: <FaUserCircle />,   label: 'Profile'  },
  ];
        ...
        {links.map(l => (
          <NavLink key={l.to} to={l.to}
            ...
            style={({ isActive }) => ({
              ...
              borderLeft: isActive ? '3px solid #cca876' : '3px solid transparent',
              whiteSpace: 'nowrap',
            })}>
