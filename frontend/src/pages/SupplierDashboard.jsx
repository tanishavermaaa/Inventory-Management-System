...
export default function SupplierDashboard() {
  ...
  const [form, setForm] = useState({
    name: '', category: '', supplier: me.name || '', price: '', stock: '', minThreshold: 25
  });
  ...
  const filteredProducts = products.filter(p => p.supplier === me.name);
  ...
  const fetchProducts = async (q = '') => {
    try {
      const res = await axios.get(`${BASE}?search=${q}&includeSupplierProducts=true`, { headers });
  ...
  const openAddModal = () => {
    setEditId(null);
    setForm({ name: '', category: '', supplier: me.name || '', price: '', stock: '', minThreshold: 25 });
  ...
  const openEditModal = (product) => {
    ...
    setForm({
      ...
      supplier: product.supplier || me.name || '',
      price:    product.price,
      stock:    product.stock,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const submitData = { ...form, supplier: me.name || '' };

      if (editId) {
        await axios.put(`${BASE}/${editId}`, submitData, { headers });
        showPopup('Product Updated Successfully!');
      } else {
        await axios.post(`${BASE}/add`, submitData, { headers });
  ...
  const stockColor = (qty) => {
    if (qty < 1000)          return { background: '#dc3545', color: '#fff' }; // low stock (red)
    if (qty <= 5000)         return { background: '#fd7e14', color: '#fff' }; // medium stock (orange)
    return                    { background: '#198754', color: '#fff' }; // high stock (green)
  };
    ...
    <div style={{ minHeight: '100vh', background: '#f0f2f5' }}>
          ...
          <div style={{
            ...
            <form onSubmit={handleSubmit} noValidate>
              ...
              <div className="mb-3">
                ...
                {!isNewCategory ? (
                  ...
                  >
                    ...
                    <option value="___NEW___" style={{fontWeight: 'bold', color: '#cca876'}}>+ Add New Category</option>
                  </select>
                ) : (
                  <div>
                    <input className={`form-control ${formErrors.category ? 'border-danger' : ''}`} placeholder="Enter new category name"
                      ...
                      onChange={e => {
                       ...
                       autoFocus />
                    <button type="button" className="btn btn-link p-0 mt-1" style={{fontSize: '0.85em', textDecoration: 'none', color: '#cca876'}}
                      onClick={() => {
                        setIsNewCategory(false);
                        setForm({ ...form, category: '' });
              ...
              <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
                ...
                <button type="submit" className="btn" style={{ background: '#cca876', color: '#fff' }} disabled={loading}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center',
          ...
          <h4 style={{ fontWeight: 700, margin: 0 , color:'black'}}>My Products Management</h4>
        ...
        <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap',
          ...
          <button className="btn"
            style={{ padding: '10px 24px', whiteSpace: 'nowrap', fontWeight: 600, background: '#cca876', color: '#fff' }}
            ...
            <tbody>
              {currentItems.length === 0 ? (
                ...
                <tr key={p._id} style={{ borderBottom: '1px solid #f0f0f0' }}>
                  ...
                  <td style={S.td}>
                    <button style={{ background: 'none', border: 'none', color: '#cca876',
...
const S = {
  ...
  okBtn:   { background: '#cca876', color: '#fff', border: 'none', borderRadius: '6px',
